...
import spectral_diffusercam_utils.helper_functions as fc
...
class fista_spectral_numpy():
    def __init__(self, h, mask):
        ...
        for i in range(0,h.shape[0]):
            self.H.append(np.expand_dims(np.fft.fft2((np.fft.ifftshift(self.pad(h[i]), axes = (0,1))), axes = (0,1)), -1))
            self.Hconj.append(np.conj(self.H[i])) 
    ...
    def Hfor(self, x, i):
        x = np.fft.ifft2(self.H[i]* np.fft.fft2(x, axes = (0,1)), axes = (0,1))
        x = np.sum(self.mask* self.crop(np.real(x)), 2)
        return x
    ...
    def Hadj(self, x, i):
        ...
        x = np.fft.ifft2(self.Hconj[i]*x, axes = (0,1))
        x = np.real(x)
        return x
